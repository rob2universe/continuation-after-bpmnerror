...
public class LoggerDelegate implements JavaDelegate {
  ...
  public void execute(DelegateExecution exec) {
    ...
    if (exec.getVariable("failAt").equals(activityId)) {
      ...
      businessError.setProcessInstance(exec.getProcessInstanceId());
      businessError.setActivityInstance(exec.getActivityInstanceId());
      businessError.setActivityId(exec.getCurrentActivityId());
      businessError.setErrorMessage(errorMessage);
      businessError.setErrorCode(errorCode);

      ObjectValue errorObj = Variables
          .objectValue(businessError)
          .serializationDataFormat(Variables.SerializationDataFormats.JSON)
          .setTransient(true)
          .create();
      exec.setVariable("businessError", errorObj);

      throw new BpmnError(errorCode, activityId + errorMessage);
    }
  }
